...
func Start(inputFileName string, outputFilename string) error {
	inputText, err := support.ReadFile(inputFileName)
	if err != nil {
		return fmt.Errorf("Ошибка чтения входного файла: %v", err)
	}
	replaceNewLines := support.ReplaceNewLines(inputText, "\n", "§")
	replaceAn := support.ReplaceAwithAn(replaceNewLines)
	...
	formatPunctuation := support.FormatPunctuation(fixQuotes)
	spaceAfterCharter := support.SpaceAfterCharter(formatPunctuation)
	...
	err = support.WriteToFile(outputFilename, correctComma)
	if err != nil {
